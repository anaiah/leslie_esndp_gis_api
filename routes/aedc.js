...
module.exports = (io) => {
    ...
    router.get('/loginpost/:uid/:pwd',async(req,res)=>{
        ...
        console.log('AEDC firing login with Authenticate====== ',uid, pwd,' ========')
        
        try {
            const sql =`select * from esndp_users where email=$1 and pwd=$2 and project='aedc'` 
    ...
    //====AEDC
    router.post('/newhousepost/:agentid', async (req, res) => {
        console.log('===newEmpPost() SAVING/UPDATING DATA=====', req.body);
        let status;
        ...
        try {
            ...
            if (method_id === "Reserve") {
                status = 1;
            } else {
                status = 2; // sold
            }

            // Check if house_id already exists in the table
            const existingHouse = await db.query(
                `SELECT id FROM aedc_production WHERE house_id = $1`,
                [house_id]
            );

            if (existingHouse.rows.length > 0) {
                // House ID exists, so perform an UPDATE
                const updateResult = await db.query(
                    `UPDATE aedc_production
                    SET sales_agent = $1, status = $2, customer = $3, cellphone = $4  WHERE house_id = $5`,
                    [
                        req.params.agentid,
                        status,
                        customer,
                ...
                res.json({ success: true, voice: 'Data Updated!' });

            } else {
                // House ID does not exist, so perform an INSERT
                const insertResult = await db.query(
                    `INSERT INTO aedc_production ( house_id, sales_agent, status, customer, cellphone )
                    VALUES ($1, $2, $3, $4, $5)
                    RETURNING id`,
                    [
                        house_id,
                ...
                res.json({ success: true, voice: 'Data Saved!' });
            }
    ...
    //=================AEDC GET CHART DATA FOR MTD PERFORMANCE ==============
    ...
    router.get('/getsite/:site', async(req,res)=>{
        try {
           ...
           const sql = `select a.id,
                ...
                (
                    case 
                        when d.full_name is not null then d.full_name
                        when d.full_name is null then 'No Agent'
                    end
                ) as agent,
                (CASE
                WHEN c.status = 1 THEN 'Reserved'
                WHEN c.status = 2 THEN 'Sold'
                when c.status is null then 'Available'
                end) as status,
                Upper(c.customer) as customer,
                c.cellphone,
                to_char(c.status_date, 'Mon DD, YYYY') as update_date
                from aedc_location a
                join aedc_project b on
                a.project_location  =  b.id
                left join aedc_production c on
                c.house_id = a.id
                left join esndp_users d on
                d.id = c.sales_agent
                where a.project_location = $1
                order by a.id`

            const result = await db.query(sql,[req.params.site]);
